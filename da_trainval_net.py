...
import itertools
from model.cycle_gan.utils import LambdaLR
from model.cycle_gan.models import Generator
from model.cycle_gan.models import Discriminator
from model.cycle_gan.utils import ReplayBuffer

from model.cycle_gan.utils import Logger
from model.cycle_gan.utils import weights_init_normal
...
if __name__ == '__main__':
  ...
  if args.dataset == "pascal_voc":
      ...
      args.t_imdb_name = "voc_clipart_2007_train_t"
      args.imdbval_name = "voc_clipart_2007_test_t"
  ...
  if args.net == 'vgg16':
    ...
    pdb.set_trace()
  batchsize = 1
  channum = 512#512/batchsize
  # cycle gan
  netG_A2B = Generator(channum, channum).cuda()
  netG_B2A = Generator(channum, channum).cuda()
  netD_A = Discriminator(channum).cuda()
  netD_B = Discriminator(channum).cuda()

  if args.dataset != 'cityscape':
      print ('SN weight init...')
      ...
      print ('nononono')
  # Lossess
  criterion_GAN = torch.nn.BCELoss()#MSELoss()
  ...
  for epoch in range(args.start_epoch, args.max_epochs + 1):
    ...
    if epoch % (args.lr_decay_step + 1) == 0:
        ...
        # adjust_learning_rate(optimizer_G, args.lr_decay_gamma)
        # adjust_learning_rate(optimizer_D_A, args.lr_decay_gamma)
        # adjust_learning_rate(optimizer_D_B, args.lr_decay_gamma)
    ...
    for step in range(iters_per_epoch):
      ...
      src_base_feat, tgt_base_feat, rois, cls_prob, bbox_pred, \
      ...
      real_A = src_base_feat#.view(batchsize, -1, src_base_feat.size(2), src_base_feat.size(3))
      real_B = tgt_base_feat#.view(batchsize, -1, src_base_feat.size(2), src_base_feat.size(3))

      target_real_A = Variable(torch.ones(real_A.size(2), real_A.size(3)).cuda(), requires_grad=False)
      target_real_B = Variable(torch.ones(real_B.size(2), real_B.size(3)).cuda(), requires_grad=False)
      target_fake_A = Variable(torch.zeros(real_A.size(2), real_A.size(3)).cuda(), requires_grad=False)
      target_fake_B = Variable(torch.zeros(real_B.size(2), real_B.size(3)).cuda(), requires_grad=False)
      ...
      loss_D_real = criterion_GAN(pred_real, target_real_A)

      pred_real = netD_A(same_A)
      loss_D_real1 = criterion_GAN(pred_real, target_real_A)

      pred_fake = netD_A(fake_A)
      loss_D_fake = criterion_GAN(pred_fake, target_fake_B)

      pred_B = netD_A(real_B)
      loss_BA = criterion_GAN(pred_B, target_fake_B)

      # Total loss
      loss_D_A = (loss_D_real + loss_D_real1 + loss_D_fake + loss_BA)/4.0

      ###################################

      pred_real = netD_B(real_B)
      loss_D_real = criterion_GAN(pred_real, target_real_B)

      pred_real = netD_B(same_B)
      loss_D_real1 = criterion_GAN(pred_real, target_real_B)

      pred_fake = netD_B(fake_B)
      loss_D_fake = criterion_GAN(pred_fake, target_fake_A)

      pred_A = netD_B(real_A)
      loss_AB = criterion_GAN(pred_A, target_fake_A)

      # Total loss
      loss_D_B = (loss_D_real + loss_D_real1 + loss_D_fake + loss_AB)/4.0

      loss = rpn_loss_cls.mean() + rpn_loss_box.mean() \
             + RCNN_loss_cls.mean() + RCNN_loss_bbox.mean() \
             + args.weight*(loss_D_A.mean() + loss_D_B.mean())
      loss_temp += loss.item()
      ...
      if step % args.disp_interval == 0:
        ...
        print("\t\t\trpn_cls: %.4f, rpn_box: %.4f, rcnn_cls: %.4f, rcnn_box %.4f,\n\t\t\tloss_D_A %.4f,loss_D_B %.4f\n\t\t\tloss_D_real %.4f,loss_D_real1 %.4f,loss_D_fake %.4f,loss_BA %.4f" \
            % (loss_rpn_cls, loss_rpn_box, loss_rcnn_cls, loss_rcnn_box, loss_D_A.mean(), loss_D_B.mean(),loss_D_real,loss_D_real1,loss_D_fake,loss_BA))
    ...
    # save_name2 = os.path.join(output_dir,
    #                          'NET_B2A_{}_{}_{}.pth'.format(args.session, epoch, step))
    # save_checkpoint({
    #     'session': args.session,
    #     'epoch': epoch + 1,
    #     'model': netG_B2A.module.state_dict() if args.mGPUs else netG_B2A.state_dict(),
    #     'optimizer': optimizer_G.state_dict(),
    #     'pooling_mode': cfg.POOLING_MODE,
    #     'class_agnostic': args.class_agnostic,
    # }, save_name2)
    # print('save model: {}'.format(save_name2))
